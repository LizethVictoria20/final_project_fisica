...
type ActiveView = "analyzer" | "history";
...
const App = () => {
  ...
  const [timeFormat, setTimeFormat] = useState<TimeFormat>("min_sec"); // 'min_sec' or 'seconds'
  ...
  useEffect(() => {
    try {
      ...
      if (storedHistory) {
        const parsed = JSON.parse(storedHistory) as AnalysisResult[];
        setHistory(parsed);
      }
      ...
      const storedTimeFormat = localStorage.getItem(
        "timeFormat"
      ) as TimeFormat | null;
      if (storedTimeFormat === "min_sec" || storedTimeFormat === "seconds") {
  ...
  const handleToggleTimeFormat = (format: TimeFormat) => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      const newResult = (await analyzeAudio(audioFile)) as AnalysisResult;
      setAnalysisResult(newResult);
      setHistory((prevHistory) => [
        newResult,
        ...prevHistory.filter((h) => h.fileName !== newResult.fileName),
      ]);
    } catch (err) {
      console.error(err);
      setError(
        "No se pudo procesar el archivo. Puede estar corrupto o en un formato no compatible."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRestoreFromHistory = (item: AnalysisResult) => {
          ...
          <>
            ...
            {analysisResult && (
              <section className="mb-12">
                ...
                <h2 className="text-xl font-bold text-blue-900 dark:text-white mb-4 mt-8">
