...
// Mismos tipos que en App / otros componentes
...
interface HistoryListProps {
  ...
  onRestore: (item: AnalysisResult) => void;
  onClear: () => void;
    ...
    <section className="mt-8">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-blue-100 dark:border-slate-700 overflow-hidden">
        <ul className="divide-y divide-slate-100 dark:divide-slate-700">
          {history.map((item: AnalysisResult, index: number) => (
            <li
              key={index}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Duración: {formatTime(item.duration, timeFormat)} •
                    Simetría: {item.proximityScore.toFixed(1)}%
                  </p>
                </div>
              </div>
