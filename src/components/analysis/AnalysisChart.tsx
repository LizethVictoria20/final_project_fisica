...
// Tipos (los mismos que en App.tsx)
...
const AnalysisChart = ({ result, timeFormat }: AnalysisChartProps) => {
  const svgRef = useRef<SVGSVGElement | null>(null);
  const wrapperRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (!result || !result.energyData || !svgRef.current || !wrapperRef.current)
      return;
    ...
    const area = d3
      .area<number>()
      .x((_d, i) => xScale((i / (energyData.length - 1)) * duration))
      .y0(height)
      .y1((d) => yScale(d))
    ...
    // Peaks
    peaks.forEach((peak: { time: number; energy: number }) => {
  ...
  return (
    ...
    >
      <svg
        ...
        aria-label="Visualización de la energía del audio con D3"
      ></svg>
    </div>
  );
};
