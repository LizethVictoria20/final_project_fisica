...
export const analyzeAudio = async (file: File) => {
  ...
  const goldenPoints = [mainGoldenPoint, secondaryGoldenPoint].sort(
    (a, b) => a - b
  );
  ...
  rmsChunks.forEach((c) => {
    c.energy /= maxEnergyValue;
  });

  // 3. Algoritmo de Detección de Picos
  const peaks: Peak[] = [];
  const threshold = averageEnergy * 1.2; // Debe ser al menos 20% más fuerte que el promedio
  const minPeakDistance = 5; // Mínimo 5 segundos entre picos
  ...
  // Filtrar picos para asegurar diversidad temporal
  const distinctPeaks: Peak[] = [];
  for (const peak of peaks) {
    if (distinctPeaks.length >= PEAK_COUNT) break;

    // Solo agregar si está lejos de los picos ya seleccionados
    const isDistinct = distinctPeaks.every(
  ...
  topPeaks.forEach((peak) => {
    const minDistance = Math.min(
      ...goldenPoints.map((gp) => Math.abs(peak.time - gp))
    );
  ...
  const proximityScore = Math.min(
    (totalProximity / topPeaks.length) * 100,
    100
  );
